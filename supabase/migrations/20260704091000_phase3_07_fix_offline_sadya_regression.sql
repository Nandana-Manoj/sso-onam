-- Phase 3 · Migration 07 — Fix a regression introduced by phase3_06.
--
-- phase3_04 (2026-07-03) fixed record_offline_sadya to authorize on
-- is_rep_of(p_tower_id) instead of the stale p_tower_id = app_tower_id()
-- check, so a multi-tower rep can record walk-ins for every tower they
-- manage, not just their residence tower (see that migration's comment and
-- web/tests/integration/security-rls.test.ts's "KNOWN BUG" case).
--
-- phase3_06 (2026-07-04) recreated the same function to add the sadya_open
-- gate, but branched off the pre-phase3_04 source and reintroduced the old
-- app_tower_id() check — silently reverting yesterday's fix. This migration
-- restores is_rep_of() while keeping the sadya_open gate.
